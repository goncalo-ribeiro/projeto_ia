...
public class ForkliftPuzzleState extends State implements Cloneable {
    ...
    public boolean canMoveForward(int idPeça) {
        ...
        if (coordenadaMaisAFrente.getColuna() == 5
            ...
            //System.out.println("can't moveforward");
            return false;
        }
        //System.out.println("can moveforward");
        return true;
    }
    ...
    */
        public double computeAmountOfBlocksInPath() {
            double h = 0;
            for (int j = columnForkLift; j < matrix.length-1; j++) {
               if (j!=columnForkLift) {
            ...
            System.out.println("Custo da heuristica = "+h);
            return h;
        }
...
/*
    public double computeAmountOfBlocksInPath() {
        //Late we gonnna have to replace martix.length-1 for martix.length
        double h = columnForkLift;
        double sequencialLose=0;
        for (int j = 0; j < matrix.length-1; j++) {
            if (j!=columnForkLift) {
                if((matrix[lineForkLift][j])!=0) {
                    //////////////////////////////////////////////////////////////////////////////////
                    //Tier 1
                    //Both side Count the same  < 1 >
                    //Puny lose
                    //h-=1;
                    //Max Lose
                    //h-=Double.NEGATIVE_INFINITY;
                    //Sequencial Lose(1st mistakes 1, 2mistakes lose 2, 3mistakes lose 3...)
                    //sequencialLose++;
                    //h-=sequencialLose;
                            /*The Further the Lose(lose based proporcional to the distance of the fork)
                                0 0 1 A 0
                                2 1   1 2*/
                    //h-= Math.abs(columnForkLift-j);
                            /*Proporcional Lose(lose based inversional to the distance of the fork)
                                0 0 1 A 0
                                1 2   2 1  */
                    //h-= j-Math.abs(columnForkLift-j);
                    //Rigth Side Counts more
                    //First to Front then to back
                            /*if (columnForkLift<j){
                                h-= j-(columnForkLift-j)/2;
                            }else h-= Math.abs(columnForkLift-j);*/
                /*}else{
                    //To The Front is the Way(Doesnt care for whatever is at his back)
                        /*if (j>columnForkLift){
                            h+=((matrix.length-1)-columnForkLift)-Math.abs(columnForkLift-j);
                        }*/
                    ////////////////////////////////////////////////////////////////////////////////
                    //Tier 2(uses tier 1, partially)
                    //Puny Gain
                    //h+=1;

                    //Sequencial Lose methodology 1 (everytime u gain, the lose becomes less powerfull)
                    //To the point where if u lose to much, it will count as gain later:3 hehe (--2)=+2
                    //sequencialLose--;
                    //Sequencial Lose methodology 2
                    //sequencialLose++;
                    //h-=sequencialLose;
                /*}
            }
